...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Archcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Archcoin Signed Message:\n";
...
{
    // Archcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 10000000 blocks, which will occur approximately every 10 years
    nSubsidy >>= (nHeight / 10000000); // Archcoin: 10,000,000 blocks in ~10 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Archcoin: 1 days
static const int64 nTargetSpacing = 0.5 * 60; // Archcoin: 0.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Archcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Archcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Archcoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Archcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ArchcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ArchcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ArchcoinMiner(CWallet *pwallet)
{
    printf("ArchcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("archcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ArchcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ArchcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ArchcoinMiner, pwallet));
}
